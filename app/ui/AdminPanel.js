...
export default function AdminPanel(){
 ...
 async function status(s){await fetch('/api/admin/messages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({conversationId:selected.id,message:s,action:'status'})});loadCs();setSelected({...selected,status:s})}
 async function logout(){await fetch('/api/admin/logout',{method:'POST'});location.href='/admin/login'}
 return <div className="admin-layout">
 <aside className="sidebar"><div className="side-top"><strong>CS Admin</strong><button onClick={logout}>Keluar</button></div>{cs.map(c=><button key={c.id} className={'conversation-item '+(selected?.id===c.id?'active':'')} onClick={()=>setSelected(c)}><strong>{c.display_name}</strong><small>#{c.public_id}</small><p>{c.status==='open'?'Terbuka':'Ditutup'}</p></button>)}</aside>
 ...
 {selected.status==='open'&&<form className="composer" onSubmit={send}><input value={text} onChange={e=>setText(e.target.value)} maxLength={2000} placeholder="Balas pelanggan..." required/><button className="primary">Kirim</button></form>}</>}</section>
 </div>
}
